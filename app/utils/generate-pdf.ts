...
// Импортируем base64-кодированные шрифты
...
export async function generateInvoicePDF(data: InvoiceFormData): Promise<jsPDF> {
  try {
    ...
    try {
      ...
      // 🚀 Устанавливаем NotoSans сразу после загрузки
    ...
    doc.text(`Invoice ${data.invoice?.number || ""}`, margin + tableWidth / 2, headerY + headerHeight / 2, {
      align: "center",
      baseline: "middle",
    })
    ...
    function printWrappedText(text: string, x: number, y: number, maxWidth: number, align: "left" | "right" = "left") {
      try {
        ...
        lines.forEach((line, index) => {
          try {
            const xPos = align === "right" ? x - doc.getStringUnitWidth(line) * doc.getFontSize() : x
        ...
        console.error("Error in printWrappedText:", wrapError)
    ...
    sellerInfo.forEach((info) => {
      if (info) {
        const lines = printWrappedText(info, 15, currentY, leftColumnWidth)
    ...
    buyerInfo.forEach((info) => {
      if (info) {
        const lines = printWrappedText(info, 110, buyerY, rightColumnWidth)
    ...
    if (data.payment?.method) {
      ...
      ].filter(Boolean) as string[] // <-- подсказка типу
    ...
    // Add spacing between currency symbols and numbers
    ...
    try {
      autoTable(doc, {
        ...
        body: (data.items || []).map((item) => {
          const netPrice = (item.quantity || 0) * (item.price || 0)
          const vatAmount = (netPrice * (item.vatRate || 0)) / 100
        ...
        columnStyles: {
          ...
          6: { cellWidth: 29, overflow: "linebreak" }, // Total column
        },
        headStyles: {
      ...
      throw new Error(
        `Failed to create items table in PDF: ${tableError instanceof Error ? tableError.message : String(tableError)}`,
      )
    }

    // Calculate totals
    const netTotal = (data.items || []).reduce((sum, item) => sum + (item.quantity || 0) * (item.price || 0), 0)
    const vatTotal = (data.items || []).reduce((sum, item) => {
      const netPrice = (item.quantity || 0) * (item.price || 0)
      return sum + (netPrice * (item.vatRate || 0)) / 100
    }, 0)
    ...
    const rightEdge = pageWidth - 15 // 15mm margin
